...
public class FilePartitioner {
	...
	// need file manager
	static Vector<Path> partitionFile(Path file, int numPartitions)
			throws IOException {
		...
		for (int i = 0; i < numPartitions; i++) {
			...
			temp = new byte[((i == numPartitions - 1) ? fileArray.length
					: (1 + i) * (fileArray.length / numPartitions))
					- i
					* (fileArray.length / numPartitions)];
			//
			// System.out.println("for( j = " +
			// i*(fileArray.length/numPartitions)+ "; j < " +
			// ((i==numPartitions-1)?fileArray.length:(i+1)*(fileArray.length/numPartitions))
			// + "; j++)");
			for (int j = i * (fileArray.length / numPartitions); j < ((i == numPartitions - 1) ? fileArray.length
					: (i + 1) * (fileArray.length / numPartitions)); j++) {
		...
		try {
			// add 2 bytes to 1st one, 1 byte for sequence nuber,and 1 byte for
			// number of numbers.
			...
			for (int i = 0; i < numPartitions; i++) {
				...
				if (m.matches()) {
					...
					outputFiles.add(Paths.get(filePrefix + "." + filePostFix
							+ i + ".part"));
