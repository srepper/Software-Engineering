...
public class FileDePartitioner {
	...
	static Path departitionFile(Vector<Path> fileArray) throws Exception {
		...
		try {
			...
			for (int i = 0; i < fileArrayByte.size(); i++) {
				...
				if (fileArrayByte.get(i)[0] == Byte.valueOf("0")) {
					numberPartitions = Integer.parseInt(""
							+ fileArrayByte.get(i)[1]);
			...
			// subtract 1 byte per array for the header, and then 1 extra for
			// the double header on #1
			...
			for (int i = 0; i < fileArrayByte.size(); i++) {
				for (int j = 0; j < fileArrayByte.size(); j++) {
					if (fileArrayByte.get(j)[0] == Byte.valueOf("" + i)) {
						for (int k = (fileArrayByte.get(i)[0] == Byte
								.valueOf("0") ? 2 : 1); k < fileArrayByte
								.get(j).length; k++) {
